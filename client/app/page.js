...
export default function App() {
  ...
  useEffect(() => {
    ...
    currentSocket.on('ROOM_CREATED', ({ roomCode, logs }) => {
    ...
    currentSocket.on('HOST_JOIN_SUCCESS', ({ roomCode, teams, queue, logs }) => {
      setRoomCode(roomCode);
      setTeams(teams || {});
      setQueue(queue || []);
      setRole('HOST');
    ...
    currentSocket.on('TEAMS_UPDATED', (updatedTeams) => setTeams(updatedTeams));
    ...
    currentSocket.on('ACTIVITY_LOGS_UPDATED', (logs) => {
      setActivityLogs(logs);
    });

    currentSocket.on('HOST_ACTION_NOTICE', ({ message }) => {
      setToastMessage(message);
      setTimeout(() => setToastMessage(''), 4000);
    });

    currentSocket.on('TEAM_REMOVED', ({ teamName: removedTeam }) => {
    ...
    currentSocket.on('PLAYER_REMOVED', ({ teamName: tName, playerName: pName }) => {
      if (
    ...
    currentSocket.on('KICKED_OUT', ({ message }) => {
